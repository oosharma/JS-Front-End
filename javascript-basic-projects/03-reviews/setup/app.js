...
function setReview(val) {
  curId += val;
  if (curId > reviews.length - 1) {
    curId = curId % reviews.length;
  }
  if (curId < 0) {
    curId = (curId + reviews.length) % reviews.length;
  }
  console.log(curId)
  imgElement.src = reviews[curId].img;
  authorElement.textContent = reviews[curId].name;
  jobElement.tabIndex = reviews[curId].job;
